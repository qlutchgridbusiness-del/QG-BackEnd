...
export class DltService {
  ...
  private readonly url = process.env.DLT_API_URL;
  private readonly apiKey = process.env.DLT_API_KEY;
  private readonly senderId = process.env.DLT_SENDER_ID;
  ...
  private readonly templates = {
    ...
    BUSINESS_BOOKING_CREATED: process.env.DLT_TEMPLATE_BUSINESS_BOOKING_CREATED,
    BUSINESS_BOOKING_ACCEPTED: process.env.DLT_TEMPLATE_BUSINESS_BOOKING_ACCEPTED,
    BUSINESS_BOOKING_REJECTED: process.env.DLT_TEMPLATE_BUSINESS_BOOKING_REJECTED,
    BUSINESS_SERVICE_STARTED: process.env.DLT_TEMPLATE_BUSINESS_SERVICE_STARTED,
    BUSINESS_SERVICE_COMPLETED: process.env.DLT_TEMPLATE_BUSINESS_SERVICE_COMPLETED,
    BUSINESS_PAYMENT_COMPLETED: process.env.DLT_TEMPLATE_BUSINESS_PAYMENT_COMPLETED,
    BUSINESS_VEHICLE_DELIVERED: process.env.DLT_TEMPLATE_BUSINESS_VEHICLE_DELIVERED,
    BUSINESS_BOOKING_CANCELLED: process.env.DLT_TEMPLATE_BUSINESS_BOOKING_CANCELLED,
  };

  private canSend() {
    return !!(this.url && this.apiKey && this.senderId);
  }
    ...
    switch (event) {
      case 'BOOKING_CREATED':
        return {
          ...
          variables: [booking.id, this.webUrl],
      ...
      case 'PAYMENT_PENDING':
        return {
          ...
          variables: [String(booking.totalAmount ?? 0), booking.id],
